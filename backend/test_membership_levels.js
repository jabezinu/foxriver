...
async function testMembershipLevelIssue() {
    ...
    try {
        ...
        // Test Case 1: Rank 1 user invites someone who joins Rank 2 (higher level)
        console.log('--- Test Case 1: Rank 1 invites Rank 2 (higher level) ---');
        ...
        const v2Membership = {
            level: 'Rank 2',
            price: 2000,
            order: membershipOrder['Rank 2']
        };
        
        // Test commission calculation
        const commissions1 = await calculateAndCreateMembershipCommissions(v2User, v2Membership);
        console.log(`Rank 1 user invites Rank 2 user - Commissions: ${commissions1.length}`);
        console.log('Expected: 0 (Rank 1 should NOT get commission from higher level Rank 2)');
        console.log(`Actual: ${commissions1.length === 0 ? '✅ CORRECT' : '❌ INCORRECT'}\n`);
        
        // Test Case 2: Rank 2 user invites someone who joins Rank 1 (lower level)
        console.log('--- Test Case 2: Rank 2 invites Rank 1 (lower level) ---');
        ...
        const v1Membership = {
            level: 'Rank 1',
            price: 1000,
            order: membershipOrder['Rank 1']
        };
        
        const commissions2 = await calculateAndCreateMembershipCommissions(v1Invited, v1Membership);
        console.log(`Rank 2 user invites Rank 1 user - Commissions: ${commissions2.length}`);
        console.log('Expected: 1+ (Rank 2 SHOULD get commission from lower level Rank 1)');
        console.log(`Actual: ${commissions2.length > 0 ? '✅ CORRECT' : '❌ INCORRECT'}\n`);
        
        // Test Case 3: Rank 1 user invites Intern (lowest level)
        console.log('--- Test Case 3: Rank 1 invites Intern (lowest level) ---');
        ...
        console.log(`Rank 1 user invites Intern - Commissions: ${commissions3.length}`);
        ...
        console.log('--- Salary Test: Rank 1 user with mixed level referrals ---');
