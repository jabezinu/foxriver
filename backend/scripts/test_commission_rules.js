 ...
 * 
 * Rules to verify:
 * 1. If inviter (V1) invites someone who stays as Intern -> NO commission, NOT counted for salary
 * 2. If inviter (V1) invites someone who upgrades to V1 (equal) -> YES commission, YES counted for salary
 * 3. If inviter (V1) invites someone who upgrades to V2 (higher) -> NO commission, NOT counted for salary
 */
...
const testCommissionRules = async () => {
    try {
        ...
        // Test Scenario 1: V1 inviter with Intern referral
        console.log('=== TEST SCENARIO 1: V1 Inviter -> Intern Referral ===');
        const v1Inviter = await User.findOne({ membershipLevel: 'V1' }).limit(1);
        if (v1Inviter) {
            ...
            console.log(`V1 Inviter: ${v1Inviter.phone}`);
            ...
            console.log(`\nSalary Calculation for V1 Inviter:`);
        ...
        // Test Scenario 2: V1 inviter with V1 referral (equal level)
        console.log('=== TEST SCENARIO 2: V1 Inviter -> V1 Referral (Equal) ===');
        const v1InviterWithV1 = await User.findOne({ membershipLevel: 'V1' }).limit(1);
        if (v1InviterWithV1) {
            ...
            console.log(`V1 Inviter: ${v1InviterWithV1.phone}`);
            console.log(`V1 Referrals Count: ${v1Referrals.length}`);
            ...
            if (v1Referrals.length > 0) {
                ...
                console.log(`Commissions from this V1 referral: ${commissions.length}`);
                console.log(`Expected: Should have commissions (equal level qualifies)`);
            }
        }

        // Test Scenario 3: V1 inviter with V2+ referral (higher level)
        console.log('\n=== TEST SCENARIO 3: V1 Inviter -> V2+ Referral (Higher) ===');
        const v1InviterWithHigher = await User.findOne({ membershipLevel: 'V1' }).limit(1);
        if (v1InviterWithHigher) {
            const higherReferrals = await User.find({ 
                referrerId: v1InviterWithHigher._id, 
                membershipLevel: { $in: ['V2', 'V3', 'V4', 'V5', 'V6', 'V7', 'V8', 'V9', 'V10'] }
            });
            
            console.log(`V1 Inviter: ${v1InviterWithHigher.phone}`);
        ...
        const v1Order = membershipOrder['V1'];
        const internOrder = membershipOrder['Intern'];
        const v2Order = membershipOrder['V2'];
        
        console.log(`V1 Order: ${v1Order}`);
        console.log(`Intern Order: ${internOrder}`);
        console.log(`V2 Order: ${v2Order}`);
        console.log('');
        console.log(`For V1 inviter (order ${v1Order}):`);
        console.log(`  - Intern referral (order ${internOrder}): Excluded by Intern check (Expected: DOES NOT QUALIFY)`);
        console.log(`  - V1 referral (order ${v1Order}): ${v1Order <= v1Order ? 'QUALIFIES' : 'DOES NOT QUALIFY'} (Expected: QUALIFIES)`);
        console.log(`  - V2 referral (order ${v2Order}): ${v2Order <= v1Order ? 'QUALIFIES' : 'DOES NOT QUALIFY'} (Expected: DOES NOT QUALIFY)`);
        console.log('');
